...
def all_odd(numbers):
    ...
    odds = []
    #list comprehension: if dividing by 2 is not 0, append that num to odds
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    # & gives intersection of the two sets, list() turns set into list
    list_of_commons = list(set_foods1 & set_foods2)
    list_of_commons.sort()
...
def every_other_item(items):
    ...
    """
    #slice from start to end and step = 2
    return items[::2]
...
def largest_n_items(items, n):
    ...
    while n > 0:
        #use index() and max() to specify position of max value and pop it
        max_value = items.pop(items.index(max(items)))
        largest.append(max_value)
        n -= 1
